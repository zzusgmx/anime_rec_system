...
@login_required
@require_POST
def toggle_like(request, anime_id):
    """
    异步处理动漫点赞
    - 使用AJAX请求处理
    - 需要用户登录
    ...
    if not created:
        like.delete()
        action = 'removed'
        message = f'已取消点赞《{anime.title}》'
    else:
        action = 'added'
        message = f'已成功点赞《{anime.title}》'
...
def anime_detail(request, slug):
    ...
    try:
        ...
        # 检查用户是否已收藏和评分（如果已登录）
        user_data = {'rating': 0, 'has_favorited': False, 'has_liked': False}
        ...
        if request.user.is_authenticated:
            try:
                ...
                from recommendation.models import AnimeLike
                # 检查是否已点赞
                user_data['has_liked'] = AnimeLike.objects.filter(
                    user=request.user, anime=anime
                ).exists()
            except Exception as e:
