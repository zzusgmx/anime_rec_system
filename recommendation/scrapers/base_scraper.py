...
class BaseScraper(ABC):
    ...
    def run(self, max_pages=1, incremental=True):
        """
        运行爬虫主流程
        ...
        try:
            for page in range(1, max_pages + 1):
                ...
                # 获取动漫列表
                anime_urls = self.scrape_anime_list(page)
                if not anime_urls:
                    logger.warning(f"第{page}页没有发现动漫URL，跳出循环")
                    break
        ...
        logger.info(f"爬虫完成，新增 {total_added} 部动漫")
        return total_added
    ...
    def _anime_exists(self, url):
        ...
        if anime_id:
            # 检查描述中是否包含该ID（作为外部引用）
            return Anime.objects.filter(description__contains=f"ID: {anime_id}").exists()
        ...
        try:
            with transaction.atomic():
                ...
                if 'cover_url' in data and data['cover_url']:
                    try:
                        image_content = self.fetch_url(data['cover_url']).content
